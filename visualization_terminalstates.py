...
G.add_node("T1", label="$8 , $7")
G.add_node("T2", label="$9 , $2")
G.add_node("T3", label="$6 , $8")
...
# Draw three concentric red circles (simulating springs) around each terminal state
for node in ["T1", "T2", "T3"]:
    x, y = pos[node]
    radii = [0.2]  # Different radii for concentric circles
    for radius in radii:
        spring_circle = Circle((x, y), radius=radius, color="red", fill=False, linestyle="--", linewidth=1.5)
