...
from game_configs_misbehave import game_configs
...
class LevelKSimulation_Recip_Parameter:
    "The objective is to compute the action probabilities for both players"
    "We define 3 ways of computing this which corresponds to level 0,1 and 2 reasoning"
    def __init__(self, game, beta_player1, beta_player2,theta_player2):
        """
        ...
        self.q = -1 if (self.max_joint_payoff  == self.joint_utility_Out 
    ...
    def _recip_utility(self, player, self_utility, opponent_utility):
        """Charness–Rabin utility. First assuming that there is no difference in weighting of joint payoffs when
        the player is ahead versus when the player is behind in terms of the difference in their own and the other players'
        direct payoff at a terminal state"""
        ...
        q = 0 if player == Player1 else self.q
        w  = theta * q   # clamp into [0,1]
        # weighted average of other's payoff vs your payoff
        return self_utility + w *(opponent_utility - self_utility)
    ...
    def simulate_level_2(self, player, state, depth=0, max_depth=3):
        ...
        if not self.action_probabilities[opponent]:
            self.simulate_level_1(opponent, state, depth=0, max_depth=max_depth)
        return self.simulate_level_1(player, state, depth, max_depth)
    

                # Example configuration for a single game

"Example Game to Initialize"
...
for game_name, config in game_configs.items():
   ...
   # print(simulation.action_probabilities[Player2])
    ...
    # Print Level 2 action probabilities for both players
    #print(f"Level 2 action probabilities for {game_name} - Player 1:")
    #print(simulation.action_probabilities[Player1])
    #print(f"Level 2 action probabilities for {game_name} - Player 2:")
    #print(simulation.action_probabilities[Player2])
