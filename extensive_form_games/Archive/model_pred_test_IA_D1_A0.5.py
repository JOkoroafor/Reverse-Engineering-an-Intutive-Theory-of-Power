...
def evaluate_and_plot(df_pred, model_name):
    ...
    plt.title(f"{model_name}\nP(In)  r={corr_in:.2f}, adj R²={adj_r2_in:.2f}, MSE={mse_in:.3f}")
    ...
    plt.title(f"{model_name}\nP(Right)  r={corr_right:.2f}, adj R²={adj_r2_right:.2f}, MSE={mse_right:.3f}")
    plt.legend(title="Game", bbox_to_anchor=(1.05, 1), loc="upper left")
    plt.grid(False)
    plt.tight_layout()
    plt.show()
    ...
    print(f"  P(In):  r={corr_in:.2f}, adj R²={adj_r2_in:.2f}, MSE={mse_in:.3f}")
    print(f"  P(Right): r={corr_right:.2f}, adj R²={adj_r2_right:.2f}, MSE={mse_right:.3f}\n")
