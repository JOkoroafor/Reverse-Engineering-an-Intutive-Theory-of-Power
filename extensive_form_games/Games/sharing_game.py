...
class SharingGame:
    ...
    def no_unique_states(self, player, state):
        actions = self.get_actions(state, player)
        unique_payoffs = set()
        print(f"Actions available for {player} at state {state}: {actions}")
        for action in actions:
            next_state = self.get_transition(state, action)
            if next_state is not None:
                reward = self.get_reward(next_state)
                payoff_tuple = (reward.get(Player1, 0), reward.get(Player2, 0))
                print(f"Action '{action}' leads to state {next_state} with payoff {payoff_tuple}")
                unique_payoffs.add(payoff_tuple)
        print(f"Unique payoff tuples: {unique_payoffs}")
        print(f"Number of unique states: {len(unique_payoffs)}")
        return len(unique_payoffs)
