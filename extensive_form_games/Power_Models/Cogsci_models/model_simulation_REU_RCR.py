...
common_games = sorted(set(REU_scores.keys()).intersection(set(RCR_scores.keys())))
...
correlation, p_value = pearsonr(reu_power_values, rcr_power_values)
...
plt.scatter(reu_power_values, rcr_power_values, color="blue", alpha=0.7)

# Label each point with the game name
for i, game in enumerate(common_games):
    plt.annotate(game, (reu_power_values[i], rcr_power_values[i]), fontsize=10, alpha=0.7)

# Compute line of best fit
slope, intercept = np.polyfit(reu_power_values, rcr_power_values, 1)
...
plt.ylabel("Predicted Power (RCR Model) [0-100]", fontsize=14)
plt.title(f"Scatter Plot: REU vs. RCR Power\nCorrelation: {correlation:.2f}", fontsize=16)
plt.legend()
plt.grid(False)
plt.tight_layout()
plt.show()
