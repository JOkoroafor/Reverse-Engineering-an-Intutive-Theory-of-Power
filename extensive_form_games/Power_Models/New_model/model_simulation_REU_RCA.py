...
common_games = sorted(set(REU_scores.keys()).intersection(set(RCA_scores.keys())))
...
correlation, p_value = pearsonr(reu_power_values, rca_power_values)
...
plt.scatter(reu_power_values, rca_power_values, color="blue", alpha=0.7)

# Label each point with the game name
for i, game in enumerate(common_games):
    plt.annotate(game, (reu_power_values[i], rca_power_values[i]), fontsize=10, alpha=0.7)

# Compute line of best fit
slope, intercept = np.polyfit(reu_power_values, rca_power_values, 1)
...
plt.title(f"Scatter Plot: REU vs. RCA Power\nCorrelation: {correlation:.2f}", fontsize=16)
plt.legend()
plt.grid(False)
plt.tight_layout()
plt.show()
