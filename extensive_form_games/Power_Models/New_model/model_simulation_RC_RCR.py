...
# Load RCR and RC results from CSV files
RCR_scores = read_csv(rcr_file)
RC_scores = read_csv(rc_file)

# Ensure both dictionaries contain the same games
common_games = sorted(set(RCR_scores.keys()).intersection(set(RC_scores.keys())))
...
Predicted_Power_RCR = normalize_to_100_scale(RCR_scores)
Predicted_Power_RC = normalize_to_100_scale(RC_scores)

# Extract values for correlation and plotting
rcr_power_values = [Predicted_Power_RCR[game] for game in common_games]
rc_power_values = [Predicted_Power_RC[game] for game in common_games]

# Compute Pearson correlation
correlation, p_value = pearsonr(rcr_power_values, rc_power_values)

# Print correlation result
print(f"\nPearson Correlation between Predicted Power (RCR) and Predicted Power (RC): {correlation:.2f}")
print(f"P-value: {p_value:.4f}")

# Create scatter plot: RCR Predicted Power vs. RC Predicted Power
plt.figure(figsize=(10, 6))
plt.scatter(rcr_power_values, rc_power_values, color="blue", alpha=0.7)

# Label each point with the game name
for i, game in enumerate(common_games):
    plt.annotate(game, (rcr_power_values[i], rc_power_values[i]), fontsize=10, alpha=0.7)

# Compute line of best fit
slope, intercept = np.polyfit(rcr_power_values, rc_power_values, 1)
x_range = np.linspace(min(rcr_power_values), max(rcr_power_values), 100)
...
plt.title(f"Scatter Plot: RCR vs. RC Power\nCorrelation: {correlation:.2f}", fontsize=16)
plt.legend()
plt.grid(False)
plt.tight_layout()
plt.show()
