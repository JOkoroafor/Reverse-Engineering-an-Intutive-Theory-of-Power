...
common_games = sorted(set(RC_scores.keys()).intersection(set(RCA_scores.keys())))
...
Predicted_Power_REU = normalize_to_100_scale(RC_scores)
Predicted_Power_RCA = normalize_to_100_scale(RCA_scores)

# Extract values for correlation and plotting
rc_power_values = [Predicted_Power_REU[game] for game in common_games]
rca_power_values = [Predicted_Power_RCA[game] for game in common_games]

# Compute Pearson correlation
correlation, p_value = pearsonr(rc_power_values, rca_power_values)

# Print correlation result
print(f"\nPearson Correlation between Predicted Power (REU) and Predicted Power (RCA): {correlation:.2f}")
print(f"P-value: {p_value:.4f}")

# Create scatter plot: REU Predicted Power vs. RC Predicted Power
plt.figure(figsize=(10, 6))
plt.scatter(rc_power_values, rca_power_values, color="blue", alpha=0.7)

# Label each point with the game name
for i, game in enumerate(common_games):
    plt.annotate(game, (rc_power_values[i], rca_power_values[i]), fontsize=10, alpha=0.7)

# Compute line of best fit
slope, intercept = np.polyfit(rc_power_values, rca_power_values, 1)
x_range = np.linspace(min(rc_power_values), max(rc_power_values), 100)
y_range = slope * x_range + intercept
plt.plot(x_range, y_range, linestyle="--", color="red", label="Best Fit Line")

# Customize plot
plt.xlabel("Predicted Power (RC Model) [0-100]", fontsize=14)
plt.ylabel("Predicted Power (RCA Model) [0-100]", fontsize=14)
plt.title(f"Scatter Plot: RC vs. RCA Power\nCorrelation: {correlation:.2f}", fontsize=16)
plt.legend()
plt.grid(False)
plt.tight_layout()
plt.show()
